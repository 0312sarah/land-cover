...
The PyTorch baseline supports multiple models via `model.name`:

- `unet` (baseline): constant filters, params via `num_layers`, `base_filters`, `upconv_filters`
- `unet_classic`: classic U-Net pyramid, params via `features: [64,128,256,512]`
- `unet_resnet34`: ResNet34 encoder + U-Net decoder, params via `pretrained` and `decoder_channels`

Example training configs are in:
- `configs/train/` (baseline U-Net)
- `configs/train_UnetResnet/` (unet_classic + unet_resnet34)
...
Located in `pytorch_baseline/notebook/`:
- `data_visualisation.ipynb`: dataset overview (counts, class distribution, example image/mask).
- `result_visualization.ipynb`: run comparison, qualitative results, metrics, confusion matrix.
- `result_visualization_2.ipynb`: image-level distribution analysis, MAE per class, ablation delta.
